...
インストールしたまま無効化する場合：`agy plugin disable i-have-adhd-plan`
...
インストールしたまま無効化する場合：`claude plugin disable i-have-adhd-plan`

### 常時有効（任意）

`SessionStart` フックが毎セッション開始時に完全なルールセットを読み込むため、`/i-have-adhd-plan` の入力が不要になります：
...
`$i-have-adhd-plan` と明示的に入力してスキルを呼び出します。Codex が自動で有効化することはありません。
...
新しいセッションを開始して `/i-have-adhd-plan` と入力します。そのセッション中は有効な状態が続きます。
...
ls ~/.gemini/commands           # コマンド方式: i-have-adhd-plan.toml が存在することを確認
```

または、セッション内で `/` と入力し、一覧に `i-have-adhd-plan` が表示されることを確認します。

### 更新

```bash
gemini extensions update i-have-adhd-plan    # 拡張機能方式
...
```bash
gemini extensions uninstall i-have-adhd-plan    # 拡張機能方式
rm ~/.gemini/commands/i-have-adhd-plan.toml     # コマンド方式
```
...
```bash
npx skills add BnJam/i-have-adhd-plan -a github-copilot        # このプロジェクトのみ
npx skills add BnJam/i-have-adhd-plan -a github-copilot -g     # 全プロジェクト共通
```
...
チャット入力欄に `/` と入力し、`i-have-adhd-plan` が表示されることを確認します。または：
...
または、配置先の skills ディレクトリから `i-have-adhd-plan` フォルダーを削除します。
...
`/i-have-adhd-plan` と入力します。スキルは `~/.hermes/skills/` にインストールされ、次回のセッション開始時からスラッシュコマンドとして利用可能になります。
...
tap も削除する場合：`hermes skills tap remove BnJam/i-have-adhd-plan`
...
1. `/plugins` を実行する。
2. **Custom** を選択する。
3. `https://github.com/BnJam/i-have-adhd-plan` を貼り付けて `Enter` を押す。
4. **Trust and install** を選択する。

スラッシュコマンド `/skill:i-have-adhd-plan` を使用することで、スキルを明示的に呼び出します。

### 更新

Kimi Code セッション内で `/plugins` を実行し、**I Have ADHD Plan** にカーソルを合わせて `R` を押します。

### アンインストール

Kimi Code セッション内で `/plugins` を実行し、**I Have ADHD Plan** にカーソルを合わせて `D` を押します。
...
OpenCode はこのリポジトリをサーバープラグインとして読み込みます。`.opencode/plugins/i-have-adhd-plan.mjs` が `skills/` のエントリーポイントと `/i-have-adhd-plan` コマンドを登録し、常時有効化されている場合はルールセットを注入します。また、OpenCode は `skills/` をネイティブにも読み込めるため、プラグインなしでもスキル自体は動作します（プラグインを追加すると `/i-have-adhd-plan` コマンドと常時有効化フラグが使えるようになります）。
...
OpenCode を起動して `/` と入力し、コマンド一覧に `i-have-adhd-plan` が表示されることを確認します。
...
有効化中はフッターに `● ADHD PLAN ON` と表示されます。もう一度コマンドを実行するとオフになります。明示的に指定することも可能です：
...
GitHub パッケージが一覧にあることを確認し、`/i-have-adhd-plan` と入力してフッターに `● ADHD PLAN ON` が表示されることを確認します。
...
Pi のエージェント設定ディレクトリに `~/.pi/agent/i-have-adhd-plan.json` を作成します：
...
- `alwaysOn`：すべてのセッションをルール有効の状態で開始します。従来の `.i-have-adhd-plan-always` フラグファイルも引き続き使えます。
- `hideStatus`：ステータスバーの `● ADHD PLAN ON` 表示を隠します。ルールと `/i-have-adhd-plan` コマンドは引き続き機能します。
...
`PI_CODING_AGENT_DIR` が設定されている場合は、そのディレクトリ内に `.i-have-adhd-plan-always` を配置してください。フラグを変更した後は `/reload` を実行するか、新しいセッションを開始してください。
...
新しい OMP セッションを開始し、`/i-have-adhd-plan` を実行してモードを切り替えます。
...
スキルを明示的に呼び出すには `/i-have-adhd-plan` と入力します。拡張機能をインストールしても、スキルを呼び出すまで出力の変化はありません。
...
一覧に `i-have-adhd-plan` が表示されることを確認します。
...
全プロジェクト共通で使う場合は **User** スコープ、現在のプロジェクトのみなら **Project** スコープで保存します。その後、Agent Panel で `/i-have-adhd-plan` と入力します。
...
Agent Panel で Skills マネージャーを開き、`i-have-adhd-plan` が一覧にあることを確認します。または、`/` と入力して表示されることを確認します。
...
Skills マネージャーから `i-have-adhd-plan` を削除するか、`~/.config/zed/skills/i-have-adhd-plan` を削除します。
...
```bash
npx skills add BnJam/i-have-adhd-plan                  # このプロジェクトのみ
npx skills add BnJam/i-have-adhd-plan -g               # 全プロジェクト共通
npx skills add BnJam/i-have-adhd-plan -a cursor -y     # 特定のエージェントのみ
npx skills add BnJam/i-have-adhd-plan -a opencode -y
```

新しいエージェントチャットで `/i-have-adhd-plan` と入力します。
...
```bash
npx skills remove i-have-adhd-plan
npx skills remove i-have-adhd-plan -g    # グローバルにインストールした場合
```
...
2. **明示的に呼び出す**：Claude Code や Qwen Code では `/i-have-adhd-plan`、Codex では `$i-have-adhd-plan` と入力します。そのセッション中はルールが有効になります。`stop adhd mode` または `normal mode` でオフにできます。
3. **`~/.claude/.i-have-adhd-plan-always` を作成する**（Claude Code）：`SessionStart` フックにより、すべてのセッションで最初のメッセージから完全なルールセットが読み込まれます。
...
**`/i-have-adhd-plan` が自動補完に表示されない →** エージェントを再起動してください。プラグインのインデックスは起動時に読み込まれます。

**常時有効フラグが効かない →** プラグインを更新（`claude plugin marketplace update i-have-adhd-plan`）して再起動してください。フックは起動時に読み込まれるほか、フラグの認識には `hooks/hooks.json` が含まれるバージョンが必要です。

**`claude plugin marketplace add` が失敗する →** `owner/repo` 形式を使用してください。ローカルパスを指定する場合は `.claude-plugin/` ではなくリポジトリのルートを指す必要があります。

**インストールしたのに回答に前置きが入る →** 新しいセッションを開いてください。それでも指示から逸脱する場合は、`skills/i-have-adhd-plan/SKILL.md` の文言をより厳格に調整してください。

**自分好みのルールに変更したい →** Fork して `skills/i-have-adhd-plan/SKILL.md` を編集し、自分のコピーに差し替えます：
...
再起動してから、`/i-have-adhd-plan` をもう一度呼び出してください。
