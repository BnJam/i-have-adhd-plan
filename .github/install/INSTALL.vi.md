...
Hoặc giữ nguyên cài đặt và tắt bằng `agy plugin disable i-have-adhd-plan`.
...
Hoặc giữ nguyên cài đặt và tắt bằng `claude plugin disable i-have-adhd-plan`.

### Luôn bật (không bắt buộc)

Hook `SessionStart` tải toàn bộ bộ quy tắc khi bắt đầu mỗi phiên, không cần `/i-have-adhd-plan`:
...
Gọi skill một cách rõ ràng bằng cách gõ `$i-have-adhd-plan`. Codex sẽ không tự động kích hoạt skill.
...
Bắt đầu phiên mới và gõ `/i-have-adhd-plan`. Skill sẽ bật trong suốt phiên đó.
...
Hoặc gõ `/` trong một phiên và xác nhận `i-have-adhd-plan` có trong danh sách.

### Cập nhật

```bash
gemini extensions update i-have-adhd-plan    # cách dùng extension
...
```bash
gemini extensions uninstall i-have-adhd-plan    # cách dùng extension
...
```bash
npx skills add BnJam/i-have-adhd-plan -a github-copilot        # dự án này
npx skills add BnJam/i-have-adhd-plan -a github-copilot -g     # mọi dự án
```
...
Gõ `/` trong ô chat và xác nhận `i-have-adhd-plan` xuất hiện. Hoặc:
...
Hoặc xóa thư mục `i-have-adhd-plan` khỏi thư mục skills nơi nó được cài.
...
Gõ `/i-have-adhd-plan`. The skill installs into `~/.hermes/skills/` and is exposed as a slash command at the next session start.
...
Hoặc xóa cả tap bằng `hermes skills tap remove BnJam/i-have-adhd-plan`.
...
1. Chạy `/plugins`.
2. Chọn **Custom**.
3. Dán `https://github.com/BnJam/i-have-adhd-plan` rồi nhấn Enter.
4. Chọn **Trust and install**.

Dùng lệnh slash `/skill:i-have-adhd-plan` để gọi skill một cách rõ ràng.

### Cập nhật

Trong phiên Kimi Code, chạy `/plugins`, đưa con trỏ đến **I Have ADHD Plan**, rồi nhấn `R`.

### Gỡ cài đặt

Trong phiên Kimi Code, chạy `/plugins`, đưa con trỏ đến **I Have ADHD Plan**, rồi nhấn `D`.
...
Bắt đầu phiên mới và gõ `/skill:i-have-adhd-plan`.
...
Hoặc gõ `/skill:` trong một phiên và xác nhận `i-have-adhd-plan` có trong danh sách.
...
Hoặc xóa `~/.pi/agent/skills/i-have-adhd-plan`.
...
Gõ `/i-have-adhd-plan` để gọi skill một cách rõ ràng. Chỉ cài extension sẽ không thay đổi đầu ra cho đến khi skill được gọi.
...
Xác nhận `i-have-adhd-plan` xuất hiện trong danh sách.
...
Lưu ở phạm vi **User** cho mọi dự án hoặc **Project** cho một dự án. Sau đó gõ `/i-have-adhd-plan` trong Agent Panel.
...
Mở trình quản lý Skills trong Agent Panel và xác nhận `i-have-adhd-plan` có trong danh sách. Hoặc gõ `/` và xác nhận nó xuất hiện.
...
Xóa `i-have-adhd-plan` khỏi trình quản lý Skills, hoặc xóa `~/.config/zed/skills/i-have-adhd-plan`.
...
```bash
npx skills remove i-have-adhd-plan
npx skills remove i-have-adhd-plan -g    # nếu đã cài toàn cục
```
...
2. **Bạn gọi skill một cách rõ ràng.** Gõ `/i-have-adhd-plan` trong Claude Code hoặc Qwen Code, hoặc `$i-have-adhd-plan` trong Codex. Quy tắc bật trong phiên đó. "stop adhd mode" hoặc "normal mode" sẽ tắt chúng.
3. **Bạn tạo `~/.claude/.i-have-adhd-plan-always`** (Claude Code). Hook `SessionStart` tải toàn bộ quy tắc từ tin nhắn đầu tiên trong mọi phiên.
...
**`/i-have-adhd-plan` không có trong tự động hoàn thành.** Khởi động lại agent. Chỉ mục plugin được đọc khi khởi động.

**Cờ luôn bật không có tác dụng.** Cập nhật plugin (`claude plugin marketplace update i-have-adhd-plan`) và khởi động lại. Hook được đọc khi khởi động, và cờ cần phiên bản plugin có `hooks/hooks.json`.
...
**Đã cài nhưng phản hồi vẫn có lời mở đầu.** Mở phiên mới. Nếu vẫn lệch, hãy làm chặt hơn cách diễn đạt trong `skills/i-have-adhd-plan/SKILL.md`.

**Muốn quy tắc khác.** Fork repo, sửa `skills/i-have-adhd-plan/SKILL.md`, rồi chuyển sang bản của bạn:
...
Khởi động lại rồi gọi `/i-have-adhd-plan` lần nữa.
