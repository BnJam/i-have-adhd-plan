...
def validate_package_manifest() -> None:
    ...
    assert package.get("pi") == {
        "extensions": ["./extensions/i-have-adhd-plan.ts"],
        "skills": ["./skills"],
    }
...
def status_texts(events: list[dict[str, Any]]) -> list[str | None]:
    return [
        ...
        and event.get("statusKey") == "i-have-adhd-plan"
    ]
...
def latest_enabled(entries_response: dict[str, Any]) -> bool:
    states = [
        ...
        and entry.get("customType") == "i-have-adhd-plan-state"
    ]
    if not states or not isinstance(states[-1], bool):
        raise AssertionError("No persisted i-have-adhd-plan state found")
    return states[-1]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test the i-have-adhd-plan extension without a model request."
    )
    parser.add_argument(
        "--runtime",
        choices=("pi", "omp"),
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"i-have-adhd-plan-{args.runtime}-"
    ) as agent_dir:
...
"""
        ...
        if args.runtime == "omp":
            try:
                ...
                assert "i-have-adhd-plan" in command_names
                assert "skill:i-have-adhd-plan" in command_names
                assert any(
                    "ADHD PLAN ON" in (text or "")
        ...
        try:
            ...
            assert "i-have-adhd-plan" in command_names
            assert "skill:i-have-adhd-plan" in command_names
            assert any("ADHD PLAN ON" in (text or "") for text in status_texts(startup_events))

            entries, _ = client.request("entries-startup", {"type": "get_entries"})
            assert message_count(entries, "i-have-adhd-plan-rules") == 1
            assert message_count(entries, "i-have-adhd-plan-disabled") == 0
            ...
            assert any("ADHD PLAN ON" in (text or "") for text in status_texts(reload_events))
            ...
            toggled_off, toggle_off_events = client.request(
                "toggle-off",
                {"type": "prompt", "message": "/i-have-adhd-plan"},
            )
            ...
            assert message_count(entries, "i-have-adhd-plan-disabled") == 1
            ...
            assert reloaded["success"] is True
            assert not any("ADHD PLAN ON" in (text or "") for text in status_texts(reload_events))
            ...
            assert any("ADHD PLAN ON" in (text or "") for text in status_texts(toggle_on_events))

            entries, _ = client.request("entries-toggled-on", {"type": "get_entries"})
            assert latest_enabled(entries) is True
            assert message_count(entries, "i-have-adhd-plan-rules") == 2

            explicit_off, _ = client.request(
                "explicit-off",
                {"type": "prompt", "message": "/i-have-adhd-plan off"},
            )
            assert explicit_off["success"] is True

            skill, skill_events = client.request(
                "skill-alias",
                {"type": "prompt", "message": "/skill:i-have-adhd-plan"},
            )
            ...
            assert latest_enabled(entries) is True
            assert message_count(entries, "i-have-adhd-plan-rules") == 3
        ...
        if args.runtime == "pi":
            Path(agent_dir, ".i-have-adhd-plan-always").touch()
            always_on = RpcClient(
                executable,
                env,
                "--no-session",
